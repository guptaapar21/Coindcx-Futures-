...
class Collector:
    ...
    def emit_record(self,k,resp,pair=None):
        ...
        self.writer(k).write(rec); self.count[k]+=1
    def _handlers(self):
        @self.sio.event
        def connect():
    ...
    def run(self,pairs):
        ...
        while not self.stop_event and time.monotonic()<end: self.sio.sleep(0.25)
