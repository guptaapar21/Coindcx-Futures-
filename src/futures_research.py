...
def book(d):
    ...
    bids,asks=side("bids",True),side("asks",False); out={"futures_book_valid":False}
    ...
    out.update({"futures_book_valid":True,"futures_best_bid":bb,"futures_best_ask":ba,"futures_mid_price":mid,"futures_spread_abs":ba-bb,"futures_spread_bps":(ba-bb)/mid*10000 if mid else None,"futures_microprice":(ba*bq+bb*aq)/(bq+aq) if bq+aq else mid})
    for n in (1,5,10,20):
...
def build_rows(batch):
    ...
    for s in SYMBOLS:
        ...
        for t in epochs:
            ...
            base[t]={"symbol":s,"epoch_second":t,"utc_second":datetime.fromtimestamp(t,timezone.utc).isoformat(),"open":price,"high":price,"low":price,"close":price,"futures_trade_count":int(b.get("count",0)),"futures_aggressive_buy_qty":float(b.get("buy",0)),"futures_aggressive_sell_qty":float(b.get("sell",0)),"futures_delta_qty":float(b.get("buy",0))-float(b.get("sell",0)),"futures_total_qty":float(b.get("total",0)),"futures_last_trade_price":price,"futures_book_features_status":"FUTURES_DEPTH_SNAPSHOT"}
            base[t].update(books[s].get(t,{}))
...
def aggregate(rows,bucket):
    g=defaultdict(list)
    for r in rows:
        e=int(r["epoch_second"]); g[(r["symbol"],e-e%bucket)].append(r)
    out=[]
    for (s,b),rs in sorted(g.items()):
        close=[r for r in rs if r.get("close") is not None];
        if not close:continue
        row={"symbol":s,"epoch":b,"utc":datetime.fromtimestamp(b,timezone.utc).isoformat(),"open":close[0]["open"],"high":max(r["high"] for r in close),"low":min(r["low"] for r in close),"close":close[-1]["close"],"trade_count":sum(r["futures_trade_count"] for r in rs),"futures_delta_qty":sum(r["futures_delta_qty"] for r in rs),"futures_total_qty":sum(r["futures_total_qty"] for r in rs)}; row["futures_delta_ratio"]=row["futures_delta_qty"]/row["futures_total_qty"] if row["futures_total_qty"] else None
...
def write(rows,path):
    ...
    fields=sorted({k for r in rows for k in r}) if rows else ["symbol"]
    with gzip.open(path,"wt",encoding="utf-8",newline="") as fh:
        w=csv.DictWriter(fh,fieldnames=fields); w.writeheader(); w.writerows(rows)

def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--batch",required=True); a=ap.parse_args(); b=Path(a.batch); rows=build_rows(b); mins=aggregate(rows,60); threes=aggregate(mins,3)
    ...
    (b/"research_summary.json").write_text(json.dumps({"schema_version":3,"market_mode":"FUTURES_ONLY","rows_1s":len(rows),"rows_1m":len(mins),"rows_3m":len(threes),"symbols":sorted({r["symbol"] for r in rows}),"windows_seconds":list(WINDOWS),"forward_horizons_seconds":list(HORIZONS)},indent=2)); print(len(rows),len(mins),len(threes))
